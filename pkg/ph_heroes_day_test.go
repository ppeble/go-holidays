...
func TestPH_NationalHeroesDay_LastMondayOfAugust(t *testing.T) {
	...
	for _, y := range sundayAug31Years {
		assertHeroesDay(t, opts, time.Date(y, time.August, 25, 0, 0, 0, 0, time.UTC), true)
		assertHeroesDay(t, opts, time.Date(y, time.September, 1, 0, 0, 0, 0, time.UTC), false)
	}
	...
	for y, mday := range controls {
		assertHeroesDay(t, opts, time.Date(y, time.August, mday, 0, 0, 0, 0, time.UTC), true)
	}
}
...
func assertHeroesDay(t *testing.T, opts holidays.Options, date time.Time, want bool) {
	t.Helper()
	hs, err := holidays.On(date, opts)
	if err != nil {
		t.Fatalf("On(%s): %v", date.Format("2006-01-02"), err)
	}
	got := false
	for _, h := range hs {
	...
	if got != want {
		t.Errorf("National Heroes Day on %s: got present=%v, want %v",
			date.Format("2006-01-02"), got, want)
	}
}
