...
- **CODE** is the installed gem (`holidays`, pinned to 11.4.0 via `Gemfile`): its
  real resolution logic.
- **DATA** is our `definitions/` submodule (v8.2.0 YAML), loaded into the running
...
```
parity_test.go  --NDJSON request-->  oracle.rb  (gem 11.4.0 + our v8.2.0 YAML)
   (Go side)     <--NDJSON result--             
```

- `oracle.rb` runs the gem, loads our v8.2.0 YAML, and answers one
...
- Ruby with the pinned `holidays` gem installed. The oracle activates
  `parity/Gemfile` via `bundler/setup`, so `holidays 11.4.0` must be available
  (`gem install holidays -v 11.4.0`, or `bundle install` from `parity/`).
- The `definitions/` submodule checked out (`git submodule update --init
  definitions`). The oracle loads its v8.2.0 YAML; without it the gem falls back
